...
// highlight-eligibility gate (size threshold + no-lang fallback), and
// tokenizeLines' Shiki-token -> per-line span reflow (CodeBlock.tsx renders
...
describe("isHighlightEligible", () => {
  // Decision table: {lang, size vs threshold} -> eligible?
  test("recognized language, within threshold -> eligible", () => {
    expect(isHighlightEligible("tsx", "const x = 1;")).toBe(true);
  });

  test("no recognized language -> not eligible regardless of size", () => {
    expect(isHighlightEligible(null, "hello")).toBe(false);
  });
  ...
  test("content length exactly at HIGHLIGHT_MAX_BYTES -> eligible", () => {
    ...
    expect(isHighlightEligible("tsx", content)).toBe(true);
  });
  ...
  test("content length one byte over HIGHLIGHT_MAX_BYTES -> not eligible (plain fallback)", () => {
    ...
    expect(isHighlightEligible("tsx", content)).toBe(false);
  });
});
