...
値は**入力から算出される**。人が選ぶのは `app.css` の層 0 (brand の色、中立と意味色の色相、識別族の並び) だけで、12 段はそこから CSS の相対色構文で解ける — JS で色を計算しない。light と dark は同じ段表の L の行が違うだけ。
...
決め方の全体 (入力・段・算出・検査・ブラウザ対応) は `docs/design/color-system.md`。
...
| 部品 | 持つもの | 持たないもの |
|---|---|---|
| `App` | どの姿で立つか (登録 / 認証 / まだ聞いていない / アプリ) | 並べ方・画面・読み方 |
| `Shell` | 並べ方 (バー・全体に掛かる知らせ・2 ペイン) | どの画面か、何を読むか |
| `ConnectionBar` | 接続の状態と、その操作・入口 (一覧 / 使用量) | 一覧や本文の中身 |
...
色は**既存のトークンだけ**を使う (`--accent` / `--live` / `--waiting` / `--danger` /
`--muted` / `--border` / `--surface` / `--bg` / `--text`)。新しい値を部品ごとに
足すと、後で色の体系を決める時に、部品の数だけ例外が残る。
