...
**launcher が起動しただけで、ハーネスがまだセッションを名乗っていないプロセス**は、ハーネスの行にしか居ない。端末と起動時刻があって id が無いので、行は何も開かず、端末だけを出す。
...
**端末を名乗るのは run** で、run はセッション自身の行に載っている (`peers.runs`)。なので端末の対応表は、画面が既に持っている一覧から作る (`terminalIdsBySid`)。ハーネスの行を読むのは、セッションの行を持たない 1 つのため — launcher が起動しただけで、まだ id を持たないプロセス。
