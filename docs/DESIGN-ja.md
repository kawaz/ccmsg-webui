...
**読むのは常に上限だけを指して**頼む。上限だけを名指した読みは範囲の**新しい側**を答えるので (`prev` がその 1 つ手前を名指す)、末尾から 1 頁ずつ手前へ歩ける。上限は**手元の先頭 item** (`until_id`、その item は含まない) で、返った `prev` と同じものを指す — 覚えた `prev` を使うと、窓が先頭を手放した後にそれが「もう無い item」を名指して、間に穴が開く。何も持っていない最初の 1 回だけは上限に**書かれうる何よりも後ろの時刻** (`until_at`) を置く: 上限の無い読みは transcript 全体を**始まりから**答えるので、そのままでは長い transcript の頭が出てしまう。`prev` が返らなければ、そこが transcript の始まり。
