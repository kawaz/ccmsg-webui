...
クエリは行内の空白が AND、改行が OR。ダブルクオートの句は 1 ワードで、句の中の連続空白は `\s+` に合わせる。`[Aa]` `[.*]` で大小の区別と正規表現に切り替える。**この文法は画面のもので契約にはない** — 契約は daemon と webui が交わす wire の取り決めで、人が窓に打つ文字列の読み方はそこに属さない (旧実装は parser を protocol package に置いていた)。
...
光らせ方は 2 通りある。地の文は render の時に text を切って `<mark>` を挟む (`markdown-view.tsx` の `text`)。色付け済みのコード行は span の列なので、同じ切り方を span 側に写して切り直す (`splitSpansForHighlight`)。markdown の中のコードだけは光らない — そこは `CodeBlock` が別に組み立てていて、切り直す口が無い。
