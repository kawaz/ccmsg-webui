...
`--warning-*` / `--danger-*` / `--tag-*`)。生の hex も、混色も、半透明の作り置きも
...
話者で色を変えない。誰が言ったかは名乗りが言っていて、色が言うのは型の違いだけ。
...
選ぶのは設定の画面 (`/settings`) の中の**色の section**。出ているのは層 0 の入力と face だけで、段表は出てこない — 読めることはそちらに閉じ込めてあり、人が選ぶ側には置かない。
