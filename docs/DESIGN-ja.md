...
**触ることは試すこと**で、覚えるのは「保存」を押した時だけ。離れるか読み込み直せば覚えてある色に戻るので、見比べるために動かした値が残らない。名前付きの組 (標準 / 暖色 / 寒色 / 無彩) は層 0 の入力だけを配るもので、段表を持たないから、どれを選んでも読めることは崩れない。組は face を持たない — 段が face ごとの明るさを 1 行で持っているので、1 つの組が既に両方の姿を持っている。ベース (選んだ組、または覚えてある値) と違う項には印が付き、項ごとに戻せる。
