...
選ぶのは設定の画面 (`/settings`) の中の**色の section**で、**基本と詳細の 2 段**になっている。基本に出るのは face と色相 3 つ (主語の色・自分・ユーザ) だけで、残りは既定のまま導かれる。詳細は畳んであり、開くと入力ぜんぶが並ぶ — 同じ入力が両方に出るので、どちらから動かしても動くのは同じ 1 つの値。出ているのは層 0 の入力と face だけで、段表は出てこない — 読めることはそちらに閉じ込めてあり、人が選ぶ側には置かない。
