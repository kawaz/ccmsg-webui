...
- **終了**: 2 度押し。1 度目は普通に頼み、**消えなかった時だけ**強い方を出す —
  強い方は transcript を書き切る機会ごと奪うので、画面が自分で選ばず人が決める
  (契約が `force` をそう定めている)
...
**この画面は独自のキーボードショートカットを持たない。** 畳んだ中まで探せる窓を ⌘F に載せれば 1 打鍵で開くが、その 1 打鍵はブラウザがページの中を探すために既に持っているもので、奪えば人は自分のブラウザの標準の手を失う。画面が足せるのは押す所であって、打鍵の意味ではない。

将来ショートカットを足すとしても: **既定は無効**、そして **ブラウザ標準の打鍵 (⌘F / ⌘K / ⌘L 等) と被る組み合わせには割り当てない**。
