...
| [DR-0004](DR-0004-one-state-machine-decides-what-the-screen-is.md) | 🟡 部分実装 | 画面ぜんぶの姿は `phase` 1 本が決め、遷移は 1 表にする。接続バーと設定の置き場、DR-0003 の木の根もそこから読む。姿・`App` の `switch`・接続後の帯の消滅・`stale` の再認証・ログアウトと設定 1 項は載っている。残るのは、契約 `auth.signout` を持つ **2.6.0 が registry に未公開** (依存は 2.5.1 のまま、daemon 側も未実装) であること |
