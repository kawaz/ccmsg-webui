...
| [DR-0004](DR-0004-one-state-machine-decides-what-the-screen-is.md) | ⬜ 未実装 | 画面ぜんぶの姿は `phase` 1 本が決め、遷移は 1 表にする。接続バーと設定の置き場、DR-0003 の木の根もそこから読む。ログアウトは契約の `auth.signout` で family を失効させ、消す範囲はキーの名前が答える |
