...
| [DR-0004](DR-0004-one-state-machine-decides-what-the-screen-is.md) | 🟡 部分実装 | 画面ぜんぶの姿は `phase` 1 本が決め、遷移は 1 表にする。接続バーと設定の置き場、DR-0003 の木の根もそこから読む。姿・`App` の `switch`・接続後の帯の消滅・`stale` の再認証・ログアウトと設定 1 項が載っている。残るのは daemon 側の `auth.signout` 待ちで、ログアウトの往復はまだ失敗側しか通っていない |
