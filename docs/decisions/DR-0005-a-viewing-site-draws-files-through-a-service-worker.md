...
| FV-Q9 | 大きなファイルの読み進め方 | SW が `Range` を受けた時に何バイトずつ頼むか。契約の 1 回あたりの上限 (契約 DR-0031) が上限を決めているので、選ぶのはその内側の刻み方だけ。**先読みはしない** (§2.3 で裁定済み) ので、残るのは「頼まれた範囲が上限を超えていた時に何回に割るか」 |
