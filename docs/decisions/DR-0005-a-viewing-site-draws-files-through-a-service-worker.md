...
**動く物は置かない**。配るのは proxy (hosting) で、FQDN の用意・証明書・配信は**フロントの責務**。この DR はそこに「この site は他のどれとも site が違うこと」「`ccmsg-view-<id>` のどの host も名前が引けて (wildcard の DNS) 同じ 2 つのファイルを返すこと」「頁の応答に `Clear-Site-Data: "*"` を付けること」だけを要求する。
