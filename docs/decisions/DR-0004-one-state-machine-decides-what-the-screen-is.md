...
| **ログアウト** | 契約の `auth.signout` で token family の失効を頼み (応答が refresh cookie を期限切れにする)、認証・接続・セッションに属する手元の記憶を消す | 人の好み (色 / 型ごとの表示属性) は残す | passkey からやり直す |
...
**何を消して何を残すかは、キーの名前が答える**。残るのは名前に**ユーザ・instance・sid のどれも含まない**もの = その人の読み方の好み (`ccmsg.theme`、`ccmsg.timeline.display:<main|sub>`)。消えるのはそれ以外 — 名前がユーザ・instance・sid を名乗るもの (タブ間で分け合う token、書きかけ、Files の選択、一覧の幅) と、繋ぎ先そのものの記憶 (`ccmsg.endpoint`)。好みは認証に属さないので、同じ端末を使い続ける人が light / dark と色を選び直さずに済む。端末を手放す人の好みがその端末に残るのはそのとおりで、それを消すのはブラウザのデータ削除の役目 (範囲の正本は DESIGN の「localStorage のキー規律」)。
...
| Q6 | ログアウトで人の好みを消すか | **(a) 残す**。残るのは名前にユーザ・instance・sid のどれも含まないキー、消えるのはそれ以外 (§2.6) | 好みは認証に属さない。同じ端末を使い続ける人に light / dark と色を選び直させる理由が無く、手放す端末から消すのはブラウザのデータ削除の役目。「名前が答える」形にしたのは、消す対象を別に列挙すると、キーが増えた時にその列挙だけが古くなるから |
