...
- **The name also answers what a sign-out clears** (DR-0004 §2.6). **What stays is what names no user, instance or sid** — the reading preferences above, and nothing else. **Everything else goes**: the keys that do name one (the token tabs share, `ccmsg.auth:<endpoint>:<user>`, an unsent draft, what the files tab remembers, a pane's width) and the memory of what was being reached at all (`ccmsg.endpoint`). There is no list of what to clear kept beside this, because a list is the part that would go stale as keys are added — the naming already says whose a value is, and the range is read off that
