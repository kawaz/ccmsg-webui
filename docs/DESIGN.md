...
**Reading further back names an upper bound only** (`until_uuid`, the record the oldest held item came from). An instance answers a bounded range from its **oldest** end, one page at a time, so a range wider than a page leaves items between what came back and what is held. That the two do not meet is what `next` says by naming something other than the oldest held item, and it is put on screen rather than pretended away.
