...
- **A sign-out clears all of it by default** (DR-0004 §2.6): every `ccmsg.` key at this origin, preferences included — what is left behind on a device somebody has stepped off should not be a default. **Something stays only where a setting says so** ("keep the local settings when signing out", off by default), and what it keeps is **what names no user, instance or sid** — the reading preferences above, and nothing else. **What belongs to the authentication, the connection or a session is not kept even then**: the keys that do name one (the token tabs share, `ccmsg.auth:<endpoint>:<user>`, an unsent draft, what the files tab remembers, a pane's width) and the memory of what was being reached at all (`ccmsg.endpoint`). There is no list of what to keep kept beside this, because a list is the part that would go stale as keys are added — the naming already says whose a value is, and the range is read off that
