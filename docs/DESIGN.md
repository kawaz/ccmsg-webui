...
Ordinary scrolling belongs to the browser. The body's pane is the scroll container (`src/layout/scroller.ts`), so the session list standing beside it cannot change where the body's end is. The whole transcript remains in the DOM. Each row uses `content-visibility: auto` and `contain-intrinsic-size`, allowing the browser to skip off-screen layout and paint without replacing rows or rewriting the scroll position.
