...
**A screen holds the last 1 MiB.** What it follows only grows, so without a bound a tab left open holds all of it. Past 1 MiB the oldest lines are let go of **whole lines at a time** — never mid-line, since `start` has to stay an offset a read can be asked to reach and half a line is not something the model layer can read. They are let go of only where the window grows at its end, so a page someone scrolled up for is never taken back out from under them by the read that fetched it. What was let go of is not lost: it is exactly what `transcript_read` answers, and the path that fills the start of a window which was never at the beginning is reused unchanged. The 1 MiB is the same tail an instance seeds its own fold of a transcript with, which keeps "what is at hand without asking" the same on both sides. The open/closed state of a fold on a line that went (`fold:<offset>`, `think:<offset>:<index>`) goes at the same moment: a fold whose line is gone means nothing, and a line paged back in starts from the reader's default.
