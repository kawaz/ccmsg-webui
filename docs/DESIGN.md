...
Ordinary scrolling belongs to the browser. The body's pane is the scroll container (`src/layout/scroller.ts`), so the session list standing beside it cannot change where the body's end is. Everything the window holds is in the DOM, and what bounds that is the window itself — the last mebibyte's worth, a few hundred rows. Rows are not skipped with `content-visibility: auto`: a skipped row is drawn at a guessed height and remeasured when it comes into view, and that swap moves the content under a view that has already been placed, so the same screen lands in a different place from one run to the next. Measured against the window's real size it buys nothing (identical frame times at 1800 rows in both engines), so the guess is not worth the movement.

The browser's scroll anchoring stays enabled (`overflow-anchor: auto`), preserving the line being read when content above changes. A tail sentinel participates in `scroll-snap-type: y proximity`, so a view resting at the end remains there as entries arrive. Webui writes a scroll position only in direct response to an explicit action such as paging or selecting a message. Older pages are requested by observing the leading sentinel with `IntersectionObserver`, not by watching scroll events.

The session tabs and the transcript controls remain sticky at the top of the body pane, so they stay available wherever the person has scrolled. The composer is not in the body at all — it lives in the floating window the prompt button opens, which follows the reader wherever they have scrolled (DR-0003 §2.7).
...
**A file is read in ranges and shown whole.** One reply carries at most `MAX_FILE_READ_BYTES` (contract DR-0031), so anything larger is asked for again from where the last range ended, and the ranges are joined as bytes before they are decoded once — a UTF-8 character straddling a boundary would be broken twice if each range became text on its own. Whether more follows is arithmetic (`offset` plus the bytes returned against `size`) rather than a flag, so the reader is never shown a head that reads as the whole file. A file that changes while it is being read is not policed: the contract promises no consistent snapshot across ranges, and what actually happens is an append, which refusing would make a growing file impossible to open.

**Bytes arrive even for what cannot be read as text.** `binary` says "do not read these as text", not "there is nothing here". What to draw a PDF or an image as is the drawing side's judgement, and that side is the viewing site (DR-0005), so this screen stops at saying what it is and how big.

**Each viewing is a fresh origin, and the webui sweeps up what it leaves behind** (DR-0005 §2.1, §2.5). The host suffix of the viewing site is the build-time constant `CCMSG_VIEW_SITE` (say `viewer.example.net`); every opening draws a random id and puts `https://ccmsg-view-<id>.<suffix>` in the iframe. The wall between drawn files is the origin, and reopening the same file never returns to the previous one. The id is written to a ledger (`localStorage` key `ccmsg.view:<id>`, value the last heartbeat) **before** the iframe is placed, and a heartbeat is stamped every minute while it is open. At start-up and every six hours, every id whose heartbeat is older than an hour has its boot page opened in a hidden iframe and sent a nonce-carrying "clean"; only an answer echoing that nonce removes the id from the ledger (`src/files/view-ledger.ts`). Closing a view also attempts a clean on the spot but never touches the ledger — the content is still alive on the same origin and can read the nonce. Hosting is asked for three things: a separate site, `dist-view/` served for every `ccmsg-view-*` host (wildcard DNS and TLS), and `Clear-Site-Data: "cookies", "storage"` on the boot page's response (see the hosting section of the README). Viewing is offered only when `CCMSG_VIEW_SITE` is set and the webui's own Service Worker is stamping COOP on its pages (the Service Worker section below).
...
**There is one way in, and it is the floating window** the prompt button opens (DR-0003 §2.7); nothing sits under the transcript. A fixed composer takes reading room away at all times and disappears the moment the view changes to files or status, which would make the same "send" present on some screens and absent on others.
...
**All the webui's service worker does is add `Cross-Origin-Opener-Policy: same-origin` to navigation responses** (`src/sw.ts`, DR-0005 §2.2). When what is drawn inside the viewing iframe opens a webui URL in a new window, an iOS PWA has its whole screen taken over; a new window that inherits the sandbox cannot load a document carrying COOP, so with COOP on the webui's pages every such route ends at an error page. COOP is a response header only — a meta tag cannot carry it — and leaning on the hosting would need configuration per deployment, so a service worker that ships inside the build adds it. It keeps no cache, no offline mode and no push: any of them would open a route to serving a stale page, and a duty to clear it.

**It lives at `<base>sw.js`, with the base as its scope.** Unlike the other assets its name does not carry a content hash: the scope follows from where the script sits, and the side that registers it (`src/service-worker.ts`) names the same URL every time. The build emits it as a second vite entry (`ccmsg-service-worker` in `vite.config.ts`), and the dev server answers the same spelling with `src/sw.ts` transformed — the visual tests run on the dev server, and viewing is only offered while the worker is in effect. `src/sw.ts` stays a classic script with no import and no export: the dev server hands out what it transforms in module form, and a single `export` would make it unreadable as a service worker.

**An update is noticed by the browser as a change of content at the same URL.** On every navigation into the scope the browser fetches `sw.js` again and compares the bytes; if they differ, it installs the new worker. With the default `updateViaCache: "imports"` the main script is refetched past the HTTP cache, so the hosting's cache settings do not decide it. The new worker calls `skipWaiting` on `install` and becomes active without waiting, and `clients.claim` on `activate` takes over the pages already open — the worker holds no state, so a page from an older build coming under the new worker has nothing to break. If `sw.js` has the same bytes across builds no update happens, and since the behaviour is the same too, nothing is harmed by the old one staying.

**Removing the worker takes more than publishing a build without it.** The registration stays in the browser; with `sw.js` gone from the host the update fetch merely fails, and an existing registration is not cleared by that (the specification's Update). To remove it, publish a `sw.js` that only calls `registration.unregister()`, and leave it there until it has reached everyone.

**Whether viewing is offered is decided by whether this worker is in effect** (DR-0005 §5). `serviceWorkerActive` (`src/service-worker.ts`) says whether an active worker is in place for the scope (`navigator.serviceWorker.ready`). It does not look at the page's `controller`: what is being protected is the navigation of a window the viewed content is about to open, and that is received by the scope's worker, not by whatever controls the current page (a hard-reloaded page is uncontrolled, but a window opened from it still goes through the worker). Where registration is refused (not a secure context, some private browsing modes) it steps back without a word, and the flag stays down.

**COOP changes only the windows that load a webui document at the top level.**

- Every window the webui opens itself (the terminal gateway, the llm-gateway login, the official sources, external links in Markdown) is opened with `rel="noreferrer"` or `window.open(…, "noreferrer")` and has no opener to begin with. What COOP `same-origin` does across origins is cut the opener, so there is nothing there to lose.
- The terminal iframe (the gateway's embed) and the viewing iframe are embedded, and COOP applies only to top-level browsing contexts. `postMessage` between the parent and an iframe arrives as before.
- A `BroadcastChannel` between tabs of the same origin (`src/auth/tab-share.ts`) arrives regardless of browsing context groups.
- The terminal iframe is sandboxed with `allow-popups` too, so if a URL that appears in the terminal is **the webui's own URL**, the window it opens inherits the sandbox and lands on an error page. That is the same treatment as viewed content, and it is exactly the route being closed. External URLs open as before.
