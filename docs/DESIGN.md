...
**Every read names an upper bound and nothing else.** An upper bound alone asks for the **newest** of a range, with `prev` naming what stands before it, which is what lets a screen walk back a page at a time from the end. The bound is the **oldest item held** (`until_id`, which the read excludes) — the same item a reply hands back as `prev`, and taking it from what is held keeps it right once the window has let go of its front, where a remembered `prev` would name something dropped and leave a hole behind it. Only the first read, with nothing held, names an instant later than anything that can be written (`until_at`) instead: a read with no bound at all is the whole transcript read from its **beginning**, which is the wrong end to open a long one at. A reply with no `prev` is the transcript's beginning.
